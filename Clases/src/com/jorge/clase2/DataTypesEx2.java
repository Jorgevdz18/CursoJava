package com.jorge.clase2;

public class DataTypesEx2 {
	
	public static void main(String[] args) {
		//Wrapper Classes
		
		//(Punto Flotante)
		//Tipo de dato de punto flotante (float) 32 bits
		Float floatVal = 1.0f;
		Float vloatVal2 = new Float(2.0f);
		//Tipo de dato de punto flotante (double) 64 bits
		Double doubleVal = 4.0;
		Double doubleVal2 = new Double(3.0);
		
		//(Tipo Entero)
		//Tipo de dato de tipo entero (int) 32 bits
		Integer intVal = 20;
		//Tipo de dato de punto enter (byte) 8 bits Rango de -128 a 127
		Byte byteVal = 127;
		//Tipo de dato de tipo entero (short) 16 bits
		Short shortVal = 7;
		//Tipo de dato de tipo entero (long) 64 bits
		Long longVal = 5L;
		
		//(Tipo Character % Boolean)
		//Tipo de dato tipo char, Guarda caracteres simples
		Character charVar = 'a';
		//Tipo de dato boolean, guarda valores booleanos(True o False)
		Boolean flag = true;
		
		//Tipo de dato de String
		String str = "String";
		
	}

}
